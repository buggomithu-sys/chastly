...
async function processCampaign(job: Job<{ campaignId: string; userId: string }>) {
  ...
  const pendingMessages = await prisma.message.findMany({
    ...
    orderBy: { id: 'asc' },
    take: campaign.dailyLimit - campaign.sentCount,
  });
  ...
  console.log(`[Campaign Worker] Processing ${pendingMessages.length} messages for campaign ${campaignId}`);

  // Queue messages with random delays
  let currentDelay = 0;
  const { addMessageJob } = await import('../lib/queue');

  for (const message of pendingMessages) {
    // Calculate random delay
    const randomDelay = Math.floor(
      Math.random() * (campaign.maxDelay - campaign.minDelay + 1) + campaign.minDelay
    );
